...
export LICHEE_GPU_LIB=${LICHEE_TOP_DIR}/platform/core/gpu/lib
export LICHEE_GPU_INCLUDE=${LICHEE_TOP_DIR}/platform/core/gpu/include
...
'linux-5.4	arm		gcc-linaro-7.3.1-2018.05-x86_64_arm-linux-gnueabihf.tar.xz	target-arm-linaro-7.3.tar.bz2'
'linux-5.4	arm		gcc-linaro-7.4.1-2019.02-x86_64_aarch64-linux-gnu.tar.xz	target-arm-linaro-7.4.tar.bz2'
'linux-5.4	arm		linux-arm-4.9.tar	                                       linux-arm-4.9.tar'
)
		...
		for j in "${arr[@]}" ; do
			if [ ${j} = $1 ] ; then
				...
				if [ ${arch} = $2 ] ; then
					...
					echo "LICHEE_ROOTFS===${LICHEE_ROOTFS}"
				fi
			fi
		done
	done
		...
		for j in "${arr[@]}" ; do
			if [ ${j} = $1 ] ; then
				...
				if [ ${arch} = $2 ] ; then
					...
					echo "LICHEE_COMPILER_TAR===${LICHEE_COMPILER_TAR}"
				fi
			fi
		done
	done
...
mk_error "substitute_inittab=== xbsp====xnor"
	...
	mk_info "env_cfg_dir={env_cfg_dir} ..."
...
function parse_boardconfig()
{
	...
	local cfgkeylist=(
			...
			LICHEE_FLASH      LICHEE_COMPILER_HARD_TAR LICHEE_KERN_DEFCONF_RECOVERY)
	...
	#	mk_error "BoardConfig=====11111===${LICHEE_COMPILER_TAR}"
	...
	if [ "x$LICHEE_GNUEABI" == "xgnueabihf" ] ; then
		...
		LICHEE_COMPILER_TAR=${LICHEE_COMPILER_HARD_TAR}
	fi
#	mk_error "BoardConfig=====22222===${LICHEE_COMPILER_TAR}"
...
{
	mk_info "build Qt ..."
	export S3_SYSROOT=$LICHEE_TOP_DIR/out/${LICHEE_IC}/${LICHEE_BOARD}/longan/buildroot/host/arm-buildroot-linux-gnueabi/sysroot
	cp ${LICHEE_GPU_INCLUDE}/* $S3_SYSROOT/usr/include -rf
	cp ${LICHEE_GPU_LIB}/* $S3_SYSROOT/usr/lib -rf
...
function mkbr()
{
	...
	[ $? -ne 0 ] && mk_error "prepare toolchain Failed!" && return 1
		...
		mk_info "build buildroot .${LICHEE_BR_DIR}.."
	...
	if [ ${LICHEE_LINUX_DEV} != "longan" ] ; then
		...
		if [ -n "`echo $LICHEE_KERN_VER | grep "linux-4.[49]"`" ]; then
			...
			mk_info "${LICHEE_ARCH}====rootfs_name={rootfs_name}"
...
function mk_defroot()
{
	...
	fakeroot mkfs.ubifs -m 4096 -e 258048 -c 580 -F -x zlib -r ${ROOTFS} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
...
function make_ubifs()
{
	...
	fakeroot ${LICHEE_BUILD_DIR}/bin/mkfs.ubifs \
		-m 4096 -e 258048 -c 580 -F -x zlib -r ${target_dir} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
}
function make_jffs2()
{
	local target_dir=$1
	rm -rf ${LICHEE_PLAT_OUT}/rootfs.jffs2
	fakeroot ${LICHEE_BUILD_DIR}/bin/mkfs.jffs2   -e 0x10000 -p 0xA00000  -d ${target_dir} -o ${LICHEE_PLAT_OUT}/rootfs.jffs2
	echo "${LICHEE_BUILD_DIR}/bin/mkfs.jffs2  ${target_dir}"
	echo "${LICHEE_PLAT_OUT}/rootfs.jffs2"
	...
	#             && ! [ -x /sbin/mkfs.jffs2 ] \
      #          && echo "Not Found /usr/sbin/mkfs.jffs2 or /sbin/mkfs.jffs2" \
       #         && return 1
        ...
        #mkfs.jffs2 -p -e 0x1000 -d /tmp/jffs2.dir \
...
function pack_rootfs()
{
	...
	if [ "x${LICHEE_FLASH}" = "xnor" ]; then
		...
		$LICHEE_PLATFORM_DIR/framework/auto/build_nor.sh
	else
