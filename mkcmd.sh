...
function mk_config()
{
	...
	# only for t113, can choose hard float toolchain for buildroot
...
function mk_defroot()
{
	...
	fakeroot mkfs.ubifs -m 2048 -e 129024 -c 290 -F -x zlib -r ${ROOTFS} -o ${LICHEE_PLAT_OUT}/rootfs.ubifs
...
function make_ubuntu()
{
    ...
    echo "$PARTITION_FEX rootfs.fex size is $ROOTFS_FEX_SIZE"
    echo "EXT4_SIZE=$ROOTFS_FEX_SIZE*512=$EXT4_SIZE"
    make_ext4fs -s -l $EXT4_SIZE ${LICHEE_PLAT_OUT}/rootfs.ext4  ${LICHEE_TOP_DIR}/buildroot/ubuntu
