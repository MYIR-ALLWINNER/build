...
function mkbootloader()
{
	...
	(cd ${brandy_path} && [ -x ${build_script} ] && ./${build_script} -p ${LICHEE_CHIP})
...
function  make_ext4()
{
    ...
    $LICHEE_BUILD_DIR/bin/make_ext4fs -s -l $EXT4_SIZE ${LICHEE_PLAT_OUT}/rootfs.ext4  $LICHEE_PLAT_OUT/buildroot/target/
	echo "$LICHEE_BUILD_DIR/bin/make_ext4fs -s -l $EXT4_SIZE ${LICHEE_PLAT_OUT}/rootfs.ext4  $LICHEE_PLAT_OUT/buildroot/target/"
}
